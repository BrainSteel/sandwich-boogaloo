...
int Paint( Bitmap* target, HWND win_handle, int x, int y, float elapsed );
...
LRESULT CALLBACK WindowProcedure( HWND win_handle, UINT message, WPARAM wparam, LPARAM lparam )
{
    ...
    switch ( message )
    {
        ...
        case WM_PAINT:
        {
            ...
            Paint( &main_menu, win_handle, x, y,  0 );
        }
        break;
...
int Paint( Bitmap* target, HWND win_handle, int mouse_x, int mouse_y, float elapsed )
{
    ...
    if ( device_context )
    {
        ...
        else if( state.game_mode == GamePlaying )
        {
            ...
            if ( device_context )
            {
                ...
                RenderGameState( &screen, &game_rect, &state, elapsed );
    ...
    if ( RegisterClass( &window_class ))
    {
        ...
        if ( win_handle )
        {
            ...
            while ( running )
            {
                ...
                {
                    if( !Paint( &main_menu, win_handle, state.in[0].mousex, state.in[0].mousey, frames_passed ) )
                    { goto quit; }
